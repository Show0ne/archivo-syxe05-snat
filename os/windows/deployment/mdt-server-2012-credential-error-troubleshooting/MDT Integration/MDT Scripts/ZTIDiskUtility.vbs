' // ***************************************************************************
' // 
' // Copyright (c) Microsoft Corporation.  All rights reserved.
' // 
' // Microsoft Deployment Toolkit Solution Accelerator
' //
' // File:      ZTIDiskUtility.vbs
' // 
' // Version:   6.1.2373.0
' // 
' // Purpose:	Utility functions for disk operations
' // 
' // Usage:	 <script language="VBScript" src="ZTIDiskUtility.vbs"/>
' // 
' // ***************************************************************************

Option Explicit

Const ZTIDiskPart_Script = "4acfedd2-f865-4b32-b8dd-8e7841b235c5"

Private Sub Class_Initialize
	If oDebugging.IsDebug Then
		oShell.Popup "ZTIDiskUtility.vbs Init By: " & oUtility.ScriptName, 0, "Class Initializer", 64
	End If
End Sub
	
Class ZTIDisk   ' VBSCript Wrapper Class around the WMI Win32_DiskDrive class

	Private g_Win32_DiskDrive
	
	Private Sub Class_Initialize
		If oDebugging.IsDebug Then
			oShell.Popup "ZTIDisk Init By: " & oUtility.ScriptName, InitalizerTimeout, "Class Initializer", 64
		End If
	End Sub
	
	Property Let Disk ( iDisk )
		Dim oDisk
		
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk Property Let Disk (" & iDisk & ")", _
			    QuestionTimeout,oUtility.ScriptName
				
		TestAndFail isNumeric(iDisk), 7801, "Verify iDisk index is numeric: " & iDisk
		If iDisk <> Disk or isempty(Disk) then
			g_Win32_DiskDrive = empty
			For Each oDisk in objWMI.ExecQuery( "SELECT * FROM Win32_DiskDrive WHERE Index=" & iDisk )
				oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk Property Let Disk ( iDisk ) found disk: " & oDisk.Path_, _
					QuestionTimeout,oUtility.ScriptName
				oLogging.CreateEntry "New ZTIDisk : " & oDisk.Path_ ,LogTypeInfo
				set g_Win32_DiskDrive = oDisk
				Exit Property
			next
		End if

	End property

	Property Set Disk ( oDisk )
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk Property Set Disk ( oDisk ) found disk: " & oDisk.Path_, _
				QuestionTimeout,oUtility.ScriptName
		TestAndFail isobject(oDisk), 7802, "Verify oDisk index is an object. " & varType(oDisk)
		TestAndFail ucase(oDisk.CreationClassName) = "WIN32_DISKDRIVE", 7803, "Verify Disk is correct WMI Object." & oDisk.Path_
		oLogging.CreateEntry "New ZTIDisk : " & oDisk.Path_ ,LogTypeInfo
		set g_Win32_DiskDrive = oDisk
	End Property

	Function GetPartitionCount
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk.GetPartitionCount()", QuestionTimeout,oUtility.ScriptName
		If not isempty(g_Win32_DiskDrive) then GetPartitionCount = g_Win32_DiskDrive.Partitions
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk.GetPartitionCount RETURNS: " & GetPartitionCount, QuestionTimeout,oUtility.ScriptName
	End function

	Function GetPartitions
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk.GetPartitions ENTERS..", QuestionTimeout,oUtility.ScriptName
		If not isempty(g_Win32_DiskDrive) then 
			set GetPartitions = objWMI.ExecQuery( "SELECT * FROM Win32_DiskPartition WHERE DiskIndex=" & Disk )
			oLogging.CreateEntry "GetPartitions: " & GetPartitions.Count ,LogTypeInfo
		End if
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk.GetPartitions EXITS..", QuestionTimeout,oUtility.ScriptName
	End function

	Function GetDiskPartition( iPartition )
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk.GetDiskPartition(" & iPartition & ")",QuestionTimeout,oUtility.ScriptName
		set GetDiskPartition = new ZTIDiskPartition
		GetDiskPartition.SetDiskPart Disk,iPartition
	End function

	Function isOSReady( sOSBuild )
		' Check to see if this disk is "capiable" of installing an OS
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk.isOSReady(" & sOSBuild & ") ENTERS..", QuestionTimeout,oUtility.ScriptName
		Dim oUSBDevice
		Dim oUSBController
		Dim oUSBPnPEnum
		Dim sPnPID
		If not isempty(g_Win32_DiskDrive) then
			isOSReady = g_Win32_DiskDrive.InterfaceType <> "USB" and g_Win32_DiskDrive.InterfaceType <> "1394"

			If not isOSReady Then
				Exit Function
			End if

			If len(trim(sOSBuild)) = 0 then
				' Do nothing, OS Version not specified.
			ElseIf cint(mid(ucase(sOSBuild),1,1)) > 6 then
				For Each oUSBDevice in objWMI.ExecQuery("ASSOCIATORS OF {" & g_Win32_DiskDrive.Path_ & "} WHERE AssocClass = Win32_PnPDevice")
					For Each oUSBController in objWMI.ExecQuery("ASSOCIATORS OF {" & oUSBDevice.Path_ & "} WHERE AssocClass = Win32_USBControllerDevice")
						For Each oUSBPnPEnum in objWMI.ExecQuery("ASSOCIATORS OF {" & oUSBController.Path_ & "} WHERE AssocClass = Win32_PnPDevice")
							For Each sPnPID in oUSBPnPEnum.CompatibleID
								If sPnPID = "PCI\\CC_0C0330" then
									oLogging.CreateEntry "Found USB 3.0 Ready Disk: " & g_Win32_DiskDrive.Path_ ,LogTypeInfo
									isOSReady = True ' USB 3.0
									Exit Function
								End if
							Next
						Next
					Next
				Next
			End if

		End if
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk.isOSReady(" & sOSBuild & ") EXITS..", QuestionTimeout,oUtility.ScriptName
	End function


	Property Get Disk
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk Property Get Disk ENTERS..", QuestionTimeout,oUtility.ScriptName
		If not isempty(g_Win32_DiskDrive) then Disk = cint(g_Win32_DiskDrive.Index)
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk Property Get Disk EXITS..", QuestionTimeout,oUtility.ScriptName
	End Property

	Function IsRemovable
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk.IsRemovable()", QuestionTimeout,oUtility.ScriptName
		Dim cap
		IsRemovable = False
		If not isempty(g_Win32_DiskDrive) then 
			For each Cap in g_Win32_DiskDrive.Capabilities
				If Cap = 7 then
					IsRemovable = True
				End if
			next
		End if
	End Function

	Function SizeAsString
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk.SizeAsString()", QuestionTimeout,oUtility.ScriptName
		If not isempty(g_Win32_DiskDrive) then SizeAsString = FormatLargeSize(g_Win32_DiskDrive.Size)
	End function

	Function oWMI
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk.oWMI() ENTERS..", QuestionTimeout,oUtility.ScriptName
		If not isempty(g_Win32_DiskDrive) then
			oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk.oWMI() g_Win32_DiskDrive IS NOT EMPTY..", QuestionTimeout,oUtility.ScriptName
		    set oWMI = g_Win32_DiskDrive
			oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk.oWMI() g_Win32_DiskDrive ASSIGNED..", QuestionTimeout,oUtility.ScriptName
		End If
		oDebugging.ShowMessage oUtility.ScriptName & " - ZTIDisk.oWMI() EXITS..", QuestionTimeout,oUtility.ScriptName
	End Function

	' // instance of Win32_DiskDrive
	' // {
	' // 	BytesPerSector = 512;
	' // 	Capabilities = {3, 4};
	' // 	CapabilityDescriptions = {"Random Access", "Supports Writing"};
	' // 	Caption = "XXXXXXXXX";
	' // 	ConfigManagerErrorCode = 0;
	' // 	ConfigManagerUserConfig = FALSE;
	' // 	CreationClassName = "Win32_DiskDrive";
	' // 	Description = "Disk drive";
	' // 	DeviceID = "\\\\.\\PHYSICALDRIVE1";
	' // 	FirmwareRevision = "0002";
	' // 	Index = 1;
	' // 	InterfaceType = "IDE";
	' // 	Manufacturer = "(Standard disk drives)";
	' // 	MediaLoaded = TRUE;
	' // 	MediaType = "Fixed hard disk media";
	' // 	Model = "XXXXXXXXX";
	' // 	Name = "\\\\.\\PHYSICALDRIVE1";
	' // 	Partitions = 1;
	' // 	PNPDeviceID = "IDE\\XXXXXXXXX....";
	' // 	SCSIBus = 0;
	' // 	SCSILogicalUnit = 0;
	' // 	SCSIPort = 0;
	' // 	SCSITargetId = 1;
	' // 	SectorsPerTrack = 63;
	' // 	SerialNumber = "XXXXXXXXX";
	' // 	Signature = 1234567890;
	' // 	Size = "500105249280";
	' // 	Status = "OK";
	' // 	SystemCreationClassName = "Win32_ComputerSystem";
	' // 	SystemName = "PICKETTK";
	' // 	TotalCylinders = "60801";
	' // 	TotalHeads = 255;
	' // 	TotalSectors = "976768065";
	' // 	TotalTracks = "15504255";
	' // 	TracksPerCylinder = 255;
	' // };

End Class


Class ZTIDiskPartition   ' VBSCript Wrapper Class around the WMI Win32_DiskPartition class

	Private g_Win32_DiskPartition
	Private g_Win32_LogicalDisk
	
	Private Sub Class_Initialize
		If oDebugging.IsDebug Then
			oShell.Popup "ZTIDiskPartition Init By: " & oUtility.ScriptName, InitalizerTimeout, "Class Initializer", 64
		End If
	End Sub
	
	'
	'NOTA: Esta funcion recibe un objeto tipo Win32_DiskPartition (oDiskType1) y retorna otro tipo Win32_LogicalDisk
	'
	Private Function GetLogicalDiskToPartition ( oDiskType1 )
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.GetLogicalDiskToPartition CLASS: " & oDiskType1.Path_,InfiniteTimeout,oUtility.ScriptName
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.GetLogicalDiskToPartition(" & oDiskType1.DeviceID & ")",InfiniteTimeout,oUtility.ScriptName
		Dim oDisk
		TestAndFail not isEmpty(oDiskType1), 7804, "Verify First object is set."
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.GetLogicalDiskToPartition() TestAndFail Ok!",QuestionTimeout,oUtility.ScriptName
		
		'
		' NOTA: Obtiene los asociadores de la clase Win32_DiskPartition.DeviceId="Disk #X,Partition #Y" del tipo Win32_LogicalDiskToPartition
		'       que se vinculan con Win32_LogicalDisk.DeviceId="<LetraDeUnidad>"
		'
		'     Win32_LogicalDiskToPartition <===> Win32_LogicalDiskToPartition <===> Win32_LogicalDisk.DeviceId
		'              DeviceId  <-------------------->  Antecedent
		'      "<Disk #X,Partition #Y>"                  Dependent  <-------------------->  DeviceId
		'                                                                               "<LetraDeUnidad>"
		For Each oDisk in objWMI.ExecQuery("ASSOCIATORS OF {" & oDiskType1.Path_ & "} WHERE AssocClass = Win32_LogicalDiskToPartition")
			oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.GetLogicalDiskToPartition LOOPING searching ASSOCIATORS for: " & oDisk.Path_, _
				InfiniteTimeout,oUtility.ScriptName
			set GetLogicalDiskToPartition = oDisk
			oLogging.CreateEntry "New ZTIDiskPartition : " & oDiskType1.Path_  & "    " & oDisk.Path_ , LogTypeInfo
			oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.GetLogicalDiskToPartition ASSOCIATORS FOUND: " & _
				oDisk.Caption ,InfiniteTimeout,oUtility.ScriptName
			Exit Function
		Next
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.GetLogicalDiskToPartition returns NOTHING",QuestionTimeout,oUtility.ScriptName
		set GetLogicalDiskToPartition = nothing
	End function


	Property Let Drive ( sDrive )
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Let Drive ( sDrive ) ENTERS.. ",QuestionTimeout,oUtility.ScriptName
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Let Drive ( sDrive ) for: " & sDrive,QuestionTimeout,oUtility.ScriptName
		Dim oLogicalDisk
		Dim oDiskPart

		If Drive <> left(sDrive,1) & ":" then
		
			g_Win32_LogicalDisk = empty
			g_Win32_DiskPartition = empty
			oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Let Drive(" & sDrive & ") empties <g_Win32_DiskPartition> ", _
			    QuestionTimeout,oUtility.ScriptName
				
			For Each oLogicalDisk in objWMI.ExecQuery( "SELECT * FROM Win32_LogicalDisk WHERE DeviceId ='" & left(sDrive,1) & ":" & "'" )
				oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Let Drive LOOPS For Each oLogicalDisk: " & oLogicalDisk.Caption, _
					QuestionTimeout,oUtility.ScriptName
				set g_Win32_LogicalDisk = oLogicalDisk
				set g_Win32_DiskPartition = GetLogicalDiskToPartition (g_Win32_LogicalDisk)

				oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition ATTEMPT TO show <g_Win32_DiskPartition>.. ", _
					InfiniteTimeout,oUtility.ScriptName
				If IsValidObject(g_Win32_DiskPartition) Then
						oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Let Drive(sDrive) sets LOGICAL: " & _
							g_Win32_DiskPartition.DeviceID,InfiniteTimeout,oUtility.ScriptName
				Else
					oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Let Drive returns isObject NOT VALID OBJECT", _
						InfiniteTimeout,oUtility.ScriptName
				End If

				Exit Property
			next
		End if
		
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Let Drive ( sDrive ) EXITS.. ",QuestionTimeout,oUtility.ScriptName
	End Property

	Property Set Drive ( oDrive )
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Set Drive ( oDrive ) ENTERS..",InfiniteTimeout,oUtility.ScriptName
		'oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Set Drive( oDrive )",QuestionTimeout,oUtility.ScriptName
		TestAndFail isobject(oDrive), 7805, "Verify oDisk index is an object. " & varType(oDisk)
		TestAndFail ucase(oDrive.CreationClassName) = "WIN32_LOGICALDISK", 7806, "Verify Drive is correct WMI Object." & oDrive.Path_
		set g_Win32_LogicalDisk = oDrive
		set g_Win32_DiskPartition = GetLogicalDiskToPartition (g_Win32_LogicalDisk)
		'oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Set Drive(oDrive) sets LOGICAL g_Win32_DiskPartition to: " & _
			'g_Win32_DiskPartition.DeviceID,InfiniteTimeout,oUtility.ScriptName
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Set Drive ( oDrive ) EXITS..",InfiniteTimeout,oUtility.ScriptName
	End Property

	Property Get Drive
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get Drive() ENTERS..",InfiniteTimeout,oUtility.ScriptName
		If IsValidObject(g_Win32_LogicalDisk) Then Drive = g_Win32_LogicalDisk.DeviceID
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get Drive EXITS",QuestionTimeout,oUtility.ScriptName
	End Property


	Property Get oWMIDrive( bForceMount )
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get oWMIDrive(bForceMount) ENTERS..",QuestionTimeout,oUtility.ScriptName
		Dim i 
		' It is possible that the partition is *not* assigned a drive letter.
		If bForceMount and not IsValidObject(g_Win32_LogicalDisk) then
			If IsValidObject(g_Win32_DiskPartition) Then
				RunDiskPartSilent array("Select Disk " & Disk, "Select Partition " & Partition, "Assign", "Exit")
				i = 1 
				While i < 120 and not IsValidObject(g_Win32_LogicalDisk)
					oUtility.SafeSleep 500
					set g_Win32_LogicalDisk = GetLogicalDiskToPartition (g_Win32_DiskPartition)
					i = i + 1
				WEnd
			End if
			TestAndFail not isempty(g_Win32_LogicalDisk), 7807, "Verify Disk has been mapped."
		End if
		If not isempty(g_Win32_LogicalDisk) then 
			set oWMIDrive = g_Win32_LogicalDisk
		Else
			set oWMIDrive = nothing
		End if
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get oWMIDrive(bForceMount) EXITS..",QuestionTimeout,oUtility.ScriptName
	End Property


	Function SetDiskPart( iDisk, iPartition ) 
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.SetDiskPart(iDisk, iPartition) ENTERS..",0,oUtility.ScriptName
		Dim oLogicalDisk
		Dim oDiskPart

		If iDisk <> Disk or iPartition <> Partition then
			g_Win32_LogicalDisk = empty
			g_Win32_DiskPartition = empty

			For Each oDiskPart in objWMI.ExecQuery( "SELECT * FROM Win32_DiskPartition WHERE DiskIndex=" & iDisk & " and Index=" & ( iPartition - 1 ) ) 
				set g_Win32_DiskPartition = oDiskPart
				'oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Function SetDiskPart( iDisk, iPartition ) sets g_Win32_DiskPartition to: " & _
					'g_Win32_DiskPartition.DeviceID,InfiniteTimeout,oUtility.ScriptName
				set g_Win32_LogicalDisk = GetLogicalDiskToPartition (g_Win32_DiskPartition)
				oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.SetDiskPart(iDisk, iPartition) EXITS..",0,oUtility.ScriptName
				Exit Function
			next
		End if
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.SetDiskPart(iDisk, iPartition) EXITS..",0,oUtility.ScriptName
	End function


	Property Set DiskPart ( oDisk )
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Set DiskPart(oDisk) ENTERS..",QuestionTimeout,oUtility.ScriptName
		'oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Set DiskPart(oDisk)",QuestionTimeout,oUtility.ScriptName
		TestAndFail isobject(oDisk), 7808, "Verify oDisk index is an object. " & varType(oDisk)
		TestAndFail ucase(oDisk.CreationClassName) = "WIN32_DISKPARTITION", 7809, "Verify DiskPart is correct WMI Object." & oDisk.Path_
		set g_Win32_DiskPartition = oDisk
		'oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Function SetDiskPart( oDisk ) sets g_Win32_DiskPartition to: " & _
			'g_Win32_DiskPartition.DeviceID,InfiniteTimeout,oUtility.ScriptName
		set g_Win32_LogicalDisk = GetLogicalDiskToPartition (g_Win32_DiskPartition)
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Set DiskPart(oDisk) EXITS..",QuestionTimeout,oUtility.ScriptName
	End Property

	Property Get oWMIDiskPart
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get oWMIDiskPart ENTERS..",QuestionTimeout,oUtility.ScriptName
		'oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get oWMIDiskPart",QuestionTimeout,oUtility.ScriptName
		If IsValidObject(g_Win32_DiskPartition) then
			oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get oWMIDiskPart IsValidObject IN..",QuestionTimeout,oUtility.ScriptName
			set oWMIDiskPart = g_Win32_DiskPartition
			oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get oWMIDiskPart IsValidObject OUT..",QuestionTimeout,oUtility.ScriptName
		Else
			oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get oWMIDiskPart CALLED WITH g_Win32_DiskPartition AT AN INVALID STATE..",0,oUtility.ScriptName
		End if
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get oWMIDiskPart EXITS..",QuestionTimeout,oUtility.ScriptName
	End Property


	Property Get Disk
	    oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get Disk ENTERS..",QuestionTimeout,oUtility.ScriptName
		If IsValidObject(g_Win32_DiskPartition) Then
			Disk = cint(g_Win32_DiskPartition.DiskIndex)
		Else
			oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get Disk CALLED WITH g_Win32_DiskPartition AT AN INVALID STATE..",0,oUtility.ScriptName
		End if
		'oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get Disk RETURNS " & Disk,QuestionTimeout,oUtility.ScriptName
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get Disk EXITS..",QuestionTimeout,oUtility.ScriptName
	End Property

	Property Get Partition
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get Partition ENTERS..",QuestionTimeout,oUtility.ScriptName
		If IsValidObject(g_Win32_DiskPartition) Then
			Partition = cint(g_Win32_DiskPartition.Index) + 1
		Else
			oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get Partition CALLED WITH g_Win32_DiskPartition AT AN INVALID STATE..",0,oUtility.ScriptName
		End if
		'oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get Partition RETURNS " & Partition,QuestionTimeout,oUtility.ScriptName
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition Property Get Partition EXITS..",QuestionTimeout,oUtility.ScriptName
	End Property


	Function GetDiskObject
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.GetDiskObject ENTERS..",QuestionTimeout,oUtility.ScriptName
		set GetDiskObject = new ZTIDisk
		' ZTIDisk.Disk = Disk
		GetDiskObject.Disk = Disk
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.GetDiskObject EXITS..",QuestionTimeout,oUtility.ScriptName
	End function

	Function isOSReady( sOSBuild, osFlags )
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.isOSReady( sOSBuild, osFlags ) ENTERS..",QuestionTimeout,oUtility.ScriptName
		If IsValidObject(g_Win32_DiskPartition) Then
			isOSReady = g_Win32_DiskPartition.Size / 1000 / 1000 >= GetMinimumDiskPartitionSizeMB
		End if
		oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.isOSReady( sOSBuild, osFlags ) EXITS..",QuestionTimeout,oUtility.ScriptName
	End function 


	' Rule: It is possible for a Win32_DiskPartition Object to *not* have an associated win32_LogicalDisk object
	' Rule: It is possible for a win32_LogicalDisk Object to *not* have an associated Win32_DiskPartition object

	'instance of Win32_DiskPartition
	'{
	'	BlockSize = "512";
	'	Bootable = FALSE;
	'	BootPartition = FALSE;
	'	Caption = "Disk #1, Partition #0";
	'	Description = "Installable File System";
	'	DeviceID = "Disk #1, Partition #0";
	'	DiskIndex = 1;
	'	Index = 0;
	'	Name = "Disk #1, Partition #0";
	'	NumberOfBlocks = "976766976";
	'	PrimaryPartition = TRUE;
	'	Size = "500104691712";
	'	StartingOffset = "1048576";
	'	Type = "Installable File System";
	'};


	'instance of Win32_LogicalDisk
	'{
	'	Access = 0;
	'	Caption = "C:";
	'	Compressed = FALSE;
	'	Description = "Local Fixed Disk";
	'	DeviceID = "C:";
	'	DriveType = 3;
	'	FileSystem = "NTFS";
	'	FreeSpace = "6641623040";
	'	MediaType = 12;
	'	Size = "79698063360";
	'	SupportsDiskQuotas = FALSE;
	'	SupportsFileBasedCompression = TRUE;
	'	VolumeName = "OSDisk";
	'	VolumeSerialNumber = "XXXXXXXXX";
	'};

End class


' -------------------------------------------------------------------------------------------------
'
'  Wrapper routines for objects above, they all return WMI objects
'

Function AllDiskDrivesEx( sSelectionCriteria )
	'oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.AllDiskDrivesEx(" & sSelectionCriteria & ")", _ 
		'QuestionTimeout,oUtility.ScriptName
	set AllDiskDrivesEx = objWMI.ExecQuery( "SELECT * FROM Win32_DiskDrive " & sSelectionCriteria )
End Function

Function AllDiskDrives
	'oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.AllDiskDrives()",QuestionTimeout,oUtility.ScriptName
	set AllDiskDrives = AllDiskDrivesEx(empty)
End Function


Function AllDiskPartEx ( sSelectionCriteria )
	oDebugging.ShowMessage oUtility.ScriptName & _
    	" ZTIDiskPartition.AllDiskPartEx( sSelectionCriteria ) ENTERS..",QuestionTimeout,oUtility.ScriptName
	set AllDiskPartEx = objWMI.ExecQuery( "SELECT * FROM Win32_DiskPartition " & sSelectionCriteria )
	oDebugging.ShowMessage oUtility.ScriptName & _
	    " ZTIDiskPartition.AllDiskPartEx( sSelectionCriteria ) EXITS..",QuestionTimeout,oUtility.ScriptName
End function

Function AllDiskPart
	oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.AllDiskPart ENTERS..",QuestionTimeout,oUtility.ScriptName
	set AllDiskPart = AllDiskPartEx(empty)
	oDebugging.ShowMessage oUtility.ScriptName & " ZTIDiskPartition.AllDiskPart EXITS..",QuestionTimeout,oUtility.ScriptName
End Function 


Function AllLogicalDrivesEx ( sSelectionCriteria )
	set AllLogicalDrivesEx = objWMI.ExecQuery( "SELECT * FROM Win32_LogicalDisk " & sSelectionCriteria )
End function

Function AllLogicalDrives
	set AllLogicalDrives = AllLogicalDrivesEx(empty)
End Function 


Function GetBootDriveEx ( bForced, sOSBuild, bBootFiles )
	oDebugging.ShowMessage oUtility.ScriptName & ".GetBootDriveEx(bForced, sOSBuild, bBootFiles) ENTERS..", QuestionTimeout,oUtility.ScriptName
	Dim oWMIDiskPart
	Dim oDiskPart
	Dim oDisk
	Dim oLogical
	Dim sCurrentARSetting
	Dim iRetVal

	set GetBootDriveEx = nothing
	
	' Get current Autorun status
	' HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\NoDriveTypeAutoRun 
	' 0xFF - Disables all drive types
	If oEnv("SystemDrive") <> "X:" then
		on error resume next
		sCurrentARSetting = oShell.Regread( "HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\NoDriveTypeAutoRun")
		if not Err then
			oLogging.CreateEntry "The current autorun setting is - " & sCurrentARSetting, logTypeInfo
		End if
		
		oLogging.CreateEntry "Disabling Autorun", logTypeInfo
		iRetVal = oUtility.regwrite ( "HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\NoDriveTypeAutoRun", 255 )
		On Error Goto 0
	End if 
	
	oLogging.CreateEntry "Find the boot drive (if any) [" & bForced & "] [" & sOSBuild & "] [" & bBootFiles & "]" , logTypeInfo
	For each oWMIDiskPart in AllDiskPartEx ( " WHERE Bootable = TRUE or BootPartition = TRUE " )
		oLogging.CreateEntry "Found bootable drive: " & oWMIDiskPart.Path_ , logTypeVerbose
		set oDiskPart = new ZTIDiskPartition
		set oDiskPart.DiskPart = oWMIDiskPart
		set oDisk = oDiskPart.GetDiskObject
		If oDisk.isOSReady( sOSBuild ) then
			set oLogical = oDiskPart.oWMIDrive( bForced )
			If isValidObject(oLogical) then
				If not bBootFiles then
					oLogging.CreateEntry "Found bootable drive (No Boot File Test) [ " & oLogical.DeviceID & " ]: " & oLogical.Path_ , logTypeInfo
					set GetBootDriveEx = oDiskPart
				ElseIf oFSO.FileExists( oLogical.DeviceID & "\ntldr" ) or oFSO.FileExists( oLogical.DeviceID & "\bootmgr" ) or oFSO.FileExists( oLogical.DeviceID & "\EFI\Microsoft\Boot\bootmgr.efi" ) then
					oLogging.CreateEntry "Found bootable drive [ " & oLogical.DeviceID & " ]: " & oLogical.Path_ , logTypeInfo
					set GetBootDriveEx = oDiskPart
				Else
					oLogging.CreateEntry "No boot files found: " & oLogical.Path_ , logTypeInfo
				End if
			End if
		End if
	next
	
	If GetBootDriveEx is nothing then
		oLogging.CreateEntry "No boot drives found. None.", logTypeInfo
	End if
	
	'Revert Autorun status to orginal setting
	If oEnv("SystemDrive") <> "X:" then
		if isempty(sCurrentARSetting) then
			sCurrentARSetting = 0
		End if 
		
		if  not isNull(sCurrentARSetting) then 
			oLogging.CreateEntry "Reverting autorun setting to - " & sCurrentARSetting, logTypeInfo
			iRetVal = oUtility.regwrite ( "HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\NoDriveTypeAutoRun", sCurrentARSetting )
		End if
	End if
oDebugging.ShowMessage oUtility.ScriptName & ".GetBootDriveEx(bForced, sOSBuild, bBootFiles) EXITS..", QuestionTimeout,oUtility.ScriptName
End Function


' -------------------------------------------------------------------------------------------------
'
'  Support Routines
'

Function IsValidObject( oTest )
	IsValidObject = False
	If isObject ( oTest ) then
		If not oTest is nothing then
			'oDebugging.ShowMessage oUtility.ScriptName & ".IsValidObject(" & oTest.Description & ")", QuestionTimeout,oUtility.ScriptName
			IsValidObject = True
		End if
	End if	
	'oDebugging.ShowMessage oUtility.ScriptName & ".IsValidObject(oTest) RETURNS: " & IsValidObject, QuestionTimeout,oUtility.ScriptName
End function

Function RunDiskPartSilent( aCommands )
	'oDebugging.ShowMessage oUtility.ScriptName & ".RunDiskPartSilent() FOR: " & aCommands, QuestionTimeout,oUtility.ScriptName
	Dim sDiskPart
	Dim sTempFile
	Dim sCommand

	sTempFile = oFileHandling.GetTempFile

	With oFSO.CreateTextFile(sTempFile,true,false)
		For each sCommand in aCommands
			oLogging.CreateEntry "DISKPART + " & sCommand, LogTypeInfo
			.WriteLine sCommand
		next
		.Close
	End With

	RunDiskPartSilent = oUtility.RunWithConsoleLoggingAndHidden("DiskPart.exe /s " & sTempFile)

End function 

Function RunDiskPart( aCommands )
	'oDebugging.ShowMessage oUtility.ScriptName & ".RunDiskPart() FOR: " & aCommands, QuestionTimeout,oUtility.ScriptName
	Dim sDiskPart
	Dim sTempFile
	Dim sCommand

	sTempFile = oFileHandling.GetTempFile

	With oFSO.CreateTextFile(sTempFile,true,false)
		For each sCommand in aCommands
			oLogging.CreateEntry "DISKPART + " & sCommand, LogTypeInfo
			.WriteLine sCommand
		next
		.Close
	End With

	RunDiskPart = oUtility.RunWithConsoleLogging("DiskPart.exe /s " & sTempFile)

End function 


Function FormatLargeSize( lSize )
	'oDebugging.ShowMessage oUtility.ScriptName & ".FormatLargeSize() FOR: " & lSize, QuestionTimeout,oUtility.ScriptName
	Dim i
	For i = 1 to len(" KMGTPEZY")
		If cdbl(lSize) < 1000 ^ i then
			FormatLargeSize = int(cdbl(lSize)/(1000^(i-1))) & " " & mid(" KMGTPEZY",i,1) & "B"
			Exit function
		End if
	next

End function


' -------------------------------------------------------------------------------------------------
'
'  BCD
'

Function RunBCDBoot()
	'oDebugging.ShowMessage oUtility.ScriptName & ".RunBCDBoot()", QuestionTimeout,oUtility.ScriptName
	If oEnv("SystemDrive") = "X:" then
		RunBCDBoot = RunBCDBootEx( oUtility.GetOSTargetDriveLetter & "\windows", empty )
	Else
		RunBCDBoot = RunBCDBootEx( oFileHandling.GetWindowsFolder, empty )
	End if
End function

Function RunBCDBootEx( sWindowsTarget, sParams )
	'oDebugging.ShowMessage oUtility.ScriptName & ".RunBCDBootEx() FOR: " & sWindowsTarget _
		'& " " & sParams, QuestionTimeout,oUtility.ScriptName
	Dim sProgram
	Dim iRC

	iRC = oUtility.FindFile("BCDBoot.exe",sProgram)
	TestAndFail iRC, 7811, "FindFile: BCDBoot.exe"
	sProgram = """" & sProgram & """"

	If oLogging.Debug then
		sProgram = sProgram & " /v"
	End if

	If OEnvironment.Item("UILanguage") = "" Then
		OEnvironment.Item("UILanguage") = oUtility.RegReadEx("HKCU\Control Panel\International\LocaleName",False)
	End If
	TestAndFail OEnvironment.Item("UILanguage") <> "", 7810, "Verify UILanguage is set."

	RunBCDBootEx = oUtility.RunWithConsoleLoggingAndHidden(sProgram & " " & sWindowsTarget  & " /l " & oEnvironment.Item("UILanguage") & " " & sParams)

End function


' -------------------------------------------------------------------------------------------------
'
'  Destination Disk/Partition/LogicalDrive routines
'

Function GetMinimumDiskPartitionSizeMB
	'oDebugging.ShowMessage oUtility.ScriptName & ".GetMinimumDiskPartitionSizeMB()", QuestionTimeout,oUtility.ScriptName
	GetMinimumDiskPartitionSizeMB = 15 * 1000  ' 15 GB

	If oEnvironment.Item("ImageBuild") <> "" then
		If left(oEnvironment.Item("ImageBuild"),3) = "5.1" then
			GetMinimumDiskPartitionSizeMB = 1500  ' 1.5 GB
		ElseIf left(oEnvironment.Item("ImageBuild"),3) = "5.2" then
			GetMinimumDiskPartitionSizeMB = 3 * 1000   ' 3 GB
		ElseIf oEnvironment.Item("ImageFlags") <> "" then
			If inStr(1,oEnvironment.Item("ImageFlags"),"SERVER",vbTextCompare) <> 0 then
				GetMinimumDiskPartitionSizeMB = 10 * 1000 ' 32 GB
			End if
		End if
	End if
	'oDebugging.ShowMessage oUtility.ScriptName & ".GetMinimumDiskPartitionSizeMB() RETURNS: " & GetMinimumDiskPartitionSizeMB _
		',QuestionTimeout,oUtility.ScriptName
End function

Function GetFirstPossibleSystemDrive
	oDebugging.ShowMessage oUtility.ScriptName & ".GetFirstPossibleSystemDrive ENTERS..", QuestionTimeout,oUtility.ScriptName
	Dim oWMIDisk
	Dim oDisk
	Dim oWMIDiskPart
	Dim oDiskPart

	If oEnv("SystemDrive") <> "X:" then
	
		GetFirstPossibleSystemDrive = oEnv("SystemDrive") 
		oLogging.CreateEntry "Found OS Disk: " & oEnv("SystemDrive")  , logTypeInfo
		
	Else
	
		for each oWMIDisk in AllDiskDrives
			set oDisk = new ZTIDisk
			set oDisk.Disk = oWMIDisk
			TestAndFail not isempty(oDisk.oWMI), 7814, "Verify class created:  " & oWMIDisk.Path_
			If oDisk.isOSReady( oEnvironment.Item("ImageBuild") ) then

				oLogging.CreateEntry "Found Possible OS TargetDisk: " & oWMIDisk.Path_ , logTypeInfo
				for each oWMIDiskPart in oDisk.GetPartitions
					oLogging.CreateEntry "Found Possible OS Target Partition: " & oWMIDiskPart.Path_ , logTypeInfo
					set oDiskPart = new ZTIDiskPartition
					set oDiskPart.DiskPart = oWMIDiskPart
					TestAndFail not isempty(oDiskPart.oWMIDiskPart), 7815,"Verify WMI Object was accepted by ZTIDiskPartition"
					If not oDiskPart.isOSReady( oenvironment.Item("ImageBuild"), oenvironment.Item("ImageFlags") ) then
						oLogging.CreateEntry "Target Partition not big enough: " & oWMIDiskPart.Path_ , logTypeInfo
					ElseIf not isEmpty(GetFirstPossibleSystemDrive) then
						oLogging.CreateEntry "Found More than one candidate OS TargetDisk: " & oWMIDiskPart.Path_ , logTypeInfo
					ElseIf not isEmpty(oDiskPart.Drive) then
						oLogging.CreateEntry "Found Drive: " & oDiskPart.Drive , logTypeInfo
						GetFirstPossibleSystemDrive = oDiskPart.Drive
					Else
						oLogging.CreateEntry "Partition was found, however no drive was associated (not formatted?) Skip: " & oWMIDiskPart.Path_ , logTypeInfo
					End if
				next
			Else
				oLogging.CreateEntry "Found Possible NonOS Disk: " & oWMIDisk.Path_ , logTypeInfo
			End if
		next
	
	End if


	If not isEmpty(GetFirstPossibleSystemDrive) then
			oLogging.CreateEntry "Found FirstPossibleSystemDrive: " & GetFirstPossibleSystemDrive , logTypeInfo
	Else
			oLogging.CreateEntry "Did not find disk." , logTypeInfo
	End if

	oDebugging.ShowMessage oUtility.ScriptName & ".GetFirstPossibleSystemDrive EXITS..", QuestionTimeout,oUtility.ScriptName
End function



' -------------------------------------------------------------------------------------------------
'
'  Legacy Routines
'


Function MarkActive(sDrive)
	'oDebugging.ShowMessage oUtility.ScriptName & ".MarkActive()", QuestionTimeout,oUtility.ScriptName
	Dim oDrive

	Set oDrive = new ZTIDiskPartition
	oDrive.Drive = sDrive
	If not isEmpty( oDrive.Drive ) then
		MarkActive = RunDiskPartSilent ( array ( "List Vol", "Select Disk " & oDrive.Disk , "Select Partition " & oDrive.Partition , "Active", "Detail Part", "Exit" ) )
	Else
		MarkActive = RunDiskPartSilent ( array ( "List Vol", "Select Vol " & sDrive, "Active",  "Detail Part", "Exit" ) )
	End if

End Function


Function GetDiskPartitionCount (iDisk)
	'oDebugging.ShowMessage oUtility.ScriptName & ".GetDiskPartitionCount()", QuestionTimeout,oUtility.ScriptName
	Dim oDisk

	set oDisk = new ZTIDisk
	oDisk.Disk = iDisk
	GetDiskPartitionCount = oDisk.GetPartitionCount

End Function


Function GetNotActiveDrive
	'oDebugging.ShowMessage oUtility.ScriptName & ".GetNotActiveDrive()", QuestionTimeout,oUtility.ScriptName
	Dim oDrive
	Dim oWMIDiskPart

	For each oWMIDiskPart in AllDiskPartEx(" WHERE BootPartition = FALSE ")
		Set oDrive = new ZTIDiskPartition
		set oDrive.DiskPart = oWMIDiskPart
		If not isEmpty(oDrive.Drive) then
			GetNotActiveDrive = oDrive.Drive

			exit Function
		End If
	next

	GetNotActiveDrive = False

End Function


function GetDiskForDrive (sDriveLetter)
	'oDebugging.ShowMessage oUtility.ScriptName & ".GetDiskForDrive()", QuestionTimeout,oUtility.ScriptName
	Dim oDrive

	Set oDrive = new ZTIDiskPartition
	oDrive.Drive = sDriveLetter
	GetDiskForDrive = oDrive.Disk
	If isEmpty(GetDiskFOrDrive) then GetDiskForDrive = -1 ' Backwards compatibility
	'oDebugging.ShowMessage oUtility.ScriptName & ".GetDiskForDrive() EXITS", QuestionTimeout,oUtility.ScriptName
End Function


Function GetPartitionSize (sDriveLetter)
	'oDebugging.ShowMessage oUtility.ScriptName & ".GetPartitionSize()", QuestionTimeout,oUtility.ScriptName
	Dim oDrive

	GetPartitionSize = 0
	Set oDrive = new ZTIDiskPartition
	oDrive.Drive = sDriveLetter
	If not isEmpty( oDrive.oWMIDrive(false) ) then
		GetPartitionSize = oDrive.oWMIDrive(false).Size
	End if

End Function


Function GetDiskSize (iDrive)
	'oDebugging.ShowMessage oUtility.ScriptName & ".GetDiskSize()", QuestionTimeout,oUtility.ScriptName
	Dim oDisk

	GetDiskSize = -1
	set oDisk = new ZTIDisk
	oDisk.Disk = iDrive
	If not isempty(oDisk.Disk) then
		GetDiskSize = cLng(Fix(oDisk.oWMI.Size / 1024 /1024))
	End if

End Function


Function GetBootDrive
	Dim oBootDrive
	'oDebugging.ShowMessage oUtility.ScriptName & ".GetBootDrive()", QuestionTimeout,oUtility.ScriptName
	set oBootDrive = GetBootDriveEx( false, "0.0.0.0", false )
	If not oBootDrive is nothing then 
		GetBootDrive = oBootDrive.Drive
	Else
		GetBootDrive = Failure
	End if

End function


' -------------------------------------------------------------------------------------------------
'
'  Routines that should be phased out.
'



Function GetDiskFreeSpace ( iDisk )
	'oDebugging.ShowMessage oUtility.ScriptName & ".GetDiskFreeSpace()", QuestionTimeout,oUtility.ScriptName
	oLogging.CreateEntry "ZTIDiskUtility!GetDiskFreeSpace should be deprecated, does not handle avaible space for a new partition", LogTypeDeprecated

	Dim oDisk
	Dim oWMIDiskPart
	Dim oDiskPart
	Dim iSum

	set oDisk = new ZTIDisk
	oDisk.Disk = iDisk

	For each oWMIDiskPart in oDisk.GetPartitions
		set oDiskPart = new ZTIDiskPartition
		set oDiskPart.DiskPart = oWMIDiskPart 
		iSum = iSum + clng(oDiskPart.oWMIDiskPart.Size / 1024 /1024)
	Next

	GetDiskFreeSpace = clng(oDisk.oWMI.Size / 1024 /1024) - iSum

End function


' Determine drive status as compared to the custom partition config
'
' Return:
'   0 = Partition exists and meets criteria
'   1 = Partition exists and does not meet the criteria
'   -1= Partition does not exist.

Function MatchPartitionConfiguration (iDriveIndex, iPartitionIndex, sDriveLetter, iMinSizeMB)
	'oDebugging.ShowMessage oUtility.ScriptName & ".MatchPartitionConfiguration()", QuestionTimeout,oUtility.ScriptName
	Dim oDiskPart
	Dim oWMIDrv

	oLogging.CreateEntry "ZTIDiskUtility!MatchPartitionConfiguration should be deprecated. Legacy Solution.", LogTypeDeprecated

	set oDiskPart = new ZTIDiskPartition
	oDiskPart.SetDiskPart iDriveIndex, iPartitionIndex + 1
	MatchPartitionConfiguration = -1
	If not isEmpty(oDiskPart.Drive) then
		MatchPartitionConfiguration = FAILURE
		set oWMIDrv = oDiskPart.oWMIDrive(false)
		If not oWMIDrv is nothing then 
			If UCase(oDiskPart.Drive) = UCase(sDriveLetter) And oWMIDrv.DriveType = 3 THen
				If (oWMIDrv.FileSystem = "FAT" Or oWMIDrv.FileSystem = "FAT32" Or oWMIDrv.FileSystem = "NTFS") Then
					If cLng(oWMIDrv.Size / 1024 / 1024) >= cLng(iMinSizeMB) Then
						MatchPartitionConfiguration = SUCCESS
					End if
				End if
			End if
		End If
	End if

End Function


